...
const FullChatScreen = ({ route, navigation }: any) => {
    const { chatId, friendName, friendId, photoURL: friendPhoto } = route.params;
    ...
    useEffect(() => {
        (async () => {
            if (currentUser) {
                ...
                setUserPhoto(
                    data?.photoURL || data?.imageUrl || data?.imageurl ||
            ...
            <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
                <View className="flex-1">
                    <View className="pt-16 pb-4 px-4 bg-[#121212] border-b border-gray-800 flex-row justify-between items-center">
                        <Text className="text-white text-xl font-semibold">{friendName}</Text>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item }) => {
                            ...
                            const profileImage = isMe ? userPhoto : friendPhoto;

                            return (
                                <View
                                    className={`px-3 my-2 ${isMe ? 'items-end' : 'items-start'
                                        }`}
                                >
